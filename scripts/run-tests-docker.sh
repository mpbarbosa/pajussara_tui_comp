#!/usr/bin/env bash
# ==============================================================================
# Docker Test Runner for pajussara_tui_comp
# ==============================================================================
...
# ── Config ────────────────────────────────────────────────────────────────────
IMAGE_NAME="pajussara_tui_comp-test"
PACKAGE_VERSION="$(node -p "require('./package.json').version")"

echo ""
echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   pajussara_tui_comp  ·  Docker Test Run   ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════╝${NC}"
echo ""
...
# Always run with --runInBand inside Docker: parallel workers + --forceExit + ESM
# create a race where one worker's VM is torn down while another's async module
# imports are still queued (ReferenceError: import after environment torn down).
# Running serially in one process eliminates the race entirely.
JEST_ARGS="--runInBand"
